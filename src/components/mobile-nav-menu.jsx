      ...
      >
        ...
        >
          <nav className="mt-2">
            <ul className="flex flex-col">
              {navItems.map((item, index) => (
                ...
                >
                  {/* focus-ring-inset: the row spans the scroll region's full
                      width, so .focus-ring's outward box-shadow was clipped at
                      the overflow-y-auto edges; the inset outline stays inside. */}
                  <a
                    href={item.href}
                    onClick={item.onClick}
                    className={`focus-ring-inset touch-manipulation group flex min-h-14 items-center gap-4 rounded-2xl px-3 py-3.5 transition-colors duration-300 ease-apple motion-reduce:transition-none md:min-h-16 md:gap-5 ${
                      item.isPrimary
          ...
          <div className="mt-auto flex flex-col">
            ...
            >
              {/* focus-ring-inset for the same clipping reason as the nav rows
                  above: this row is also full-bleed inside the scroll region. */}
              <a
                href="https://github.com/helixnow/deep-student"
                target="_blank"
                rel="noopener noreferrer"
                className="focus-ring-inset touch-manipulation group flex min-h-12 items-center gap-3 rounded-xl px-3 py-3 text-[color:var(--apple-muted)] transition-colors duration-300 ease-apple hover:bg-[color:var(--apple-btn-secondary-bg)] hover:text-[color:var(--apple-ink)] motion-reduce:transition-none md:min-h-14"
              >
