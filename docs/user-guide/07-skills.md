...
依赖工具组：`knowledge-retrieval`、`vfs-memory`、`ask-user`
...
| `vfs-memory` | 记忆管理 | 写入/读取/更新/删除/列举长期记忆 |
...
A: 可以在对话中点击技能标签的关闭按钮关闭它，或输入 `/skill off`。但由于它是默认技能，下次新建会话时会再次自动激活。
