...
- 选择一张或多张图片（支持 PNG/JPG/JPEG/GIF/WEBP/BMP）
...
| **工具调用块** (MCP Tool) | AI 调用 MCP 工具 | 展示工具名称、输入参数、执行状态和结果 |
...
| **论文下载块** (Paper Save) | 学术搜索技能保存论文 | 展示下载进度和保存状态 |
...
2. **命令激活**：输入 `/skill [技能 ID]`（如 `/skill chatanki`）
3. **自动推荐**：在对话中描述需求，AI 可能主动建议激活相关技能
...
| `deep-student` | 深度学者 | **默认开启**，主动回忆用户记忆，个性化回答 | 统一搜索、记忆工具 |
...
| **Cmd/Ctrl + Shift + K** | 切换知识库面板 |
| **Cmd/Ctrl + Shift + M** | 切换 MCP 工具面板 |
